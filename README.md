...
Run-level feedback is exposed through `PUT /api/threads/{thread_id}/runs/{run_id}/feedback` with `result=positive|negative` and `DELETE /api/threads/{thread_id}/runs/{run_id}/feedback` to reset the SQL feedback row to the neutral placeholder (`rating=0`, `result=null`).
