...
| `PUT /api/threads/{id}/runs/{run_id}/feedback` | Set run feedback with `result=positive|negative`; `rating=1|-1` remains accepted for compatibility |
| `DELETE /api/threads/{id}/runs/{run_id}/feedback` | Reset run feedback to the neutral placeholder row (`rating=0`, `result=null`) |
...
Business tool note:
- Domain-specific skills can expose native DeerFlow tools through `tools[].use` (for example `tools.oceanengine_local_project:oceanengine_local_project_tool`) so runtime calls do not depend on sandbox shell execution.
- MCP tools owned by a native business tool should be invoked through that business tool. Direct `nacos-mcp-router_use_tool` calls for managed operations are blocked to preserve validation, unit normalization, and mutation confirmation.
