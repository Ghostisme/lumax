...
class AuthContext:
    ...
    def __init__(self, user: Any | None = None, permissions: list[str] | None = None):
        self.user = user
...
def _set_runtime_user_context_if_missing(auth_context: AuthContext) -> Any | None:
    ...
    if get_current_user() is not None:
        return None
    return set_current_user(auth_context.user)


def _reset_runtime_user_context(token: Any | None) -> None:
    if token is None:
        return
...
def require_auth[**P, T](func: Callable[P, T]) -> Callable[P, T]:
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        runtime_token = _set_runtime_user_context_if_missing(auth_context)
        try:
            return await func(*args, **kwargs)
        finally:
            _reset_runtime_user_context(runtime_token)
...
def require_permission(
    ...
    def decorator(func: Callable[P, T]) -> Callable[P, T]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            runtime_token = _set_runtime_user_context_if_missing(auth)
            try:
                # Check permission
