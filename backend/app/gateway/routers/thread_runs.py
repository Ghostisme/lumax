...
class RunFeedbackRequest(BaseModel):
    result: Literal["positive", "negative"] | None = Field(
        default=None, description="Feedback result"
    )
    rating: Literal[1, -1] | None = Field(
        default=None, description="Compatibility rating"
    )
...
def _feedback_result_from_body(body: RunFeedbackRequest) -> str:
    ...
    if body.rating == -1:
        return "negative"
    raise HTTPException(
        status_code=422, detail="Either result or rating must be provided"
    )


def _agent_name_from_run_record(record: RunRecord) -> str:
    config = record.kwargs.get("config") if isinstance(record.kwargs, dict) else None
    if not isinstance(config, dict):
        return ""
    for key in ("context", "configurable"):
        values = config.get(key)
        if isinstance(values, dict) and values.get("agent_name"):
            return str(values["agent_name"])
    return ""


async def _sync_feedback_if_enabled(
    run_id: str, result: str, comment: str | None
) -> None:
    config = get_feedback_config()
    if not config.langsmith_sync:
        return
    from app.gateway.routers.feedback import _sync_to_langsmith

    await _sync_to_langsmith(run_id, result, comment)
...
async def stream_run(
    ...
    The response includes ``Content-Location`` for run metadata and
    ``Location`` for SDK resumable-stream reconnects.
    """
    ...
    return StreamingResponse(
        ...
        headers={
            ...
            "Location": f"/api/threads/{thread_id}/runs/{record.run_id}/stream",
        },
    )
...
async def wait_run(thread_id: str, body: RunCreateRequest, request: Request) -> dict:
    ...
    try:
        ...
        if checkpoint_tuple is not None:
            ...
            return sanitize_user_visible_payload(
...
async def upsert_run_feedback(
    thread_id: str, run_id: str, body: RunFeedbackRequest, request: Request
) -> dict:
    """Create or update the current user's feedback for a run."""
    config = get_feedback_config()
    if not config.enabled:
        raise HTTPException(status_code=403, detail="Feedback is disabled")

    result = _feedback_result_from_body(body)
    if config.require_comment_on_negative and result == "negative" and not body.comment:
        raise HTTPException(
    ...
    feedback = await feedback_repo.upsert_by_run(
        run_id=run_id,
        thread_id=thread_id,
        user_id=user_id,
        result=result,
        ...
        agent_name=_agent_name_from_run_record(record),
    )
    await _sync_feedback_if_enabled(run_id, result, body.comment)
    return feedback


@router.delete("/{thread_id}/runs/{run_id}/feedback")
@require_permission("runs", "read", owner_check=True)
async def reset_run_feedback(thread_id: str, run_id: str, request: Request) -> dict:
    """Reset the current user's feedback for a run to the neutral state."""
    config = get_feedback_config()
    if not config.enabled:
        raise HTTPException(status_code=403, detail="Feedback is disabled")
    ...
    feedback_repo = get_feedback_repo(request)
    user_id = await get_current_user(request)
    feedback = await feedback_repo.reset_by_run(
        thread_id=thread_id, run_id=run_id, user_id=user_id
    )
    if feedback is None:
        raise HTTPException(
            status_code=404, detail=f"Feedback for run {run_id} not found"
        )
    return feedback
...
@router.api_route(
    "/{thread_id}/runs/{run_id}/stream", methods=["GET", "POST"], response_model=None
)
@require_permission("runs", "read", owner_check=True)
...
async def list_thread_messages(
    ...
    messages = await event_store.list_messages(
        thread_id, limit=limit, before_seq=before_seq, after_seq=after_seq
    )
            ...
            msg["feedback"] = (
                {
                    ...
                    "feedback_time": fb.get("feedback_time"),
                    "agent_id": fb.get("agent_id"),
                    "agent_name": fb.get("agent_name"),
                }
                if fb
                else None
            )
...
async def list_run_events(
    ...
    return await event_store.list_events(
        thread_id, run_id, event_types=types, limit=limit
    )
