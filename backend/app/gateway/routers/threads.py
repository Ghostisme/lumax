...
def _log_user_attr(user: Any, *names: str) -> str | None:
    for name in names:
...
async def _resolve_thread_display_name(
    row: dict[str, Any],
    *,
    checkpointer: Any,
    thread_store: Any,
) -> str | None:
    """Return a thread title, backfilling summary metadata from checkpoint if needed."""
    ...
    try:
        ...
        title = (
            channel_values.get("title") if isinstance(channel_values, dict) else None
        )
        ...
        try:
            await thread_store.update_display_name(
                thread_id, title_text, touch_updated_at=False
            )
        except Exception:
            logger.debug(
                "Failed to backfill thread title for %s (non-fatal)",
                sanitize_log_param(thread_id),
            )
        return title_text
    except Exception:
        logger.debug(
            "Failed to read checkpoint title for %s (non-fatal)",
            sanitize_log_param(thread_id),
            exc_info=True,
        )
        return None
...
async def search_threads(
    ...
    request_user_id = _log_user_attr(request_user, "user_id", "id")
    context_user_id = _log_user_attr(context_user, "user_id", "id")
    tenant_id = _log_user_attr(request_user, "tenant_id")
    business_code = _log_user_attr(request_user, "business_code")
    metadata_keys = sorted((body.metadata or {}).keys())
    logger.info(
        "thread_search_request path=%s request_user_id=%s context_user_id=%s tenant_id=%s business_code=%s status=%s limit=%s offset=%s metadata_keys=%s",
        getattr(getattr(request, "url", None), "path", "/api/threads/search"),
        request_user_id,
        context_user_id,
        tenant_id,
        business_code,
    ...
    logger.info(
        "thread_search_result request_user_id=%s context_user_id=%s row_count=%s",
        request_user_id,
        context_user_id,
        len(rows),
    )
    ...
    for row in rows:
        display_name = await _resolve_thread_display_name(
            row,
        ...
        responses.append(
            ThreadResponse(
                ...
                values={"title": display_name} if display_name else {},
                interrupts={},
            )
        )
    return responses
...
async def get_thread(thread_id: str, request: Request) -> ThreadResponse:
    ...
    return ThreadResponse(
        ...
        updated_at=str(record.get("updated_at", "")),
        metadata=record.get("metadata", {}),
        values=sanitize_user_visible_payload(serialize_channel_values(channel_values)),
    )
...
async def get_thread_history(
    thread_id: str, body: ThreadHistoryRequest, request: Request
) -> list[HistoryEntry]:
    ...
    try:
        async for checkpoint_tuple in checkpointer.alist(config, limit=body.limit):
            ...
            if is_latest_checkpoint:
                ...
                if messages:
                    values["messages"] = sanitize_user_visible_payload(
                        serialize_channel_values({"messages": messages})
                    ).get("messages", [])
