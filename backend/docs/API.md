...
DeerFlow Gateway uses platform-issued bearer tokens for non-public API paths.
Clients must send the platform access token and tenant context:

- `Authorization: Bearer <access_token>`
- `TENANT-ID` (or compatible `TenantId` / `tenant-id`)
- `BusinessCode`, `Business-Code`, `BUSINESS_CODE`, or `BUSINESS-CODE`

The Gateway validates tokens against the platform Redis token store. It first
tries the tenant/business scoped key format, then falls back to the platform
native key `token::access_token::<access_token>`.
...
1. Configure `AUTH_REDIS_URL` or `REDIS_URL` to point at the platform token Redis database
2. Keep `AUTH_ENABLED=true`
3. Use Nginx or the platform gateway to preserve `Authorization`, tenant, and business headers
