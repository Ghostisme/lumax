...
def _get_enabled_skills():
    ...
    if not event.wait(timeout=_ENABLED_SKILLS_REFRESH_WAIT_TIMEOUT_SECONDS):
        logger.warning("Timed out waiting %.1fs for enabled skills cache warm-up", _ENABLED_SKILLS_REFRESH_WAIT_TIMEOUT_SECONDS)
        return []
...
- The task required 5+ tool calls to resolvea
...
def _get_cached_skills_prompt_section(
    ...
    filtered = [(name, description, category, location) for name, description, category, location in skill_signature if
                available_skills_key is None or name in available_skills_key]
    skills_list = ""
    if filtered:
        skill_items = "\n".join(
...
def get_skills_prompt_section(available_skills: set[str] | None = None, *, app_config: AppConfig | None = None) -> str:
    ...
    skill_signature = tuple(
        (skill.name, skill.description, skill.category, skill.get_container_file_path(container_base_path)) for skill in
        skills)
...
def apply_prompt_template(
    ...
    *,
    ...
    prompt = SYSTEM_PROMPT_TEMPLATE.format(
        agent_name=agent_name or "Lumax",
