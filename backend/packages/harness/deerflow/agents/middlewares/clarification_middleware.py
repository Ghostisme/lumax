...
_OCEANENGINE_USER_HIDDEN_IDENTIFIERS = (
    "localAccountId",
    "local_account_id",
    "multi_poi_ids",
    "payload_json",
...
def _sanitize_user_clarification_text(text: str) -> str:
    if not isinstance(text, str) or not text:
        return text

    hidden_pattern = "|".join(re.escape(identifier) for identifier in _OCEANENGINE_USER_HIDDEN_IDENTIFIERS)
    sanitized = re.sub(rf"[（(][^）)]*(?:{hidden_pattern})[^）)]*[）)]", "", text)
    sanitized = re.sub(hidden_pattern, "", sanitized)
...
class ClarificationMiddleware(AgentMiddleware[ClarificationMiddlewareState]):
    ...
    def _format_clarification_message(self, args: dict) -> str:
        ...
        return _sanitize_user_clarification_text("\n".join(message_parts))
