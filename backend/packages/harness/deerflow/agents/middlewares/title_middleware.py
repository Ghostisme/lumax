...
class TitleMiddleware(AgentMiddleware[TitleMiddlewareState]):
    ...
    def _fallback_title(self, user_msg: str) -> str:
        config = get_title_config()
        title = self._summarize_user_intent(user_msg)
        if not title:
            return "New Conversation"
        return title[: config.max_chars].rstrip()

    def _summarize_user_intent(self, user_msg: str) -> str:
        text = re.sub(r"\s+", " ", user_msg).strip()
        if not text:
            return ""

        text = re.split(r"[，,。；;：:]\s*(?:信息如下|参数如下|字段如下|具体如下|如下)\b", text, maxsplit=1)[0]
        text = re.split(r"\b(?:信息如下|参数如下|字段如下|具体如下)\b", text, maxsplit=1)[0]
        text = re.split(r"[\n\r]", text, maxsplit=1)[0].strip()
        text = re.sub(r"^(?:请|麻烦|帮我|帮忙|请帮我|能否|可以)?\s*", "", text).strip()
        text = re.sub(r"^(?:创建|新建|生成|查询|更新|修改|删除)\s*一个", lambda m: m.group(0)[:-2], text)
        ...
        fallback_chars = min(get_title_config().max_chars, 50)
        if len(text) > fallback_chars:
            text = text[:fallback_chars].rstrip() + "..."
        return text
