...
from deerflow.config.feedback_config import FeedbackConfig, load_feedback_config_from_dict
from deerflow.config.memory_config import MemoryConfig, load_memory_config_from_dict
from deerflow.config.mcp_config import McpConfig, NacosConfig
...
class AppConfig(BaseModel):
    ...
    mcp: McpConfig = Field(default_factory=McpConfig, description="MCP configuration from config.yaml")
    nacos: NacosConfig = Field(default_factory=NacosConfig, description="Nacos and nacos-mcp-router configuration")
    tool_search: ToolSearchConfig = Field(default_factory=ToolSearchConfig, description="Tool search / deferred loading configuration")
    title: TitleConfig = Field(default_factory=TitleConfig, description="Automatic title generation configuration")
    summarization: SummarizationConfig = Field(default_factory=SummarizationConfig, description="Conversation summarization configuration")
    memory: MemoryConfig = Field(default_factory=MemoryConfig, description="Memory subsystem configuration")
    feedback: FeedbackConfig = Field(default_factory=FeedbackConfig, description="User feedback configuration")
    ...
    def from_file(cls, config_path: str | None = None) -> Self:
        ...
        # Load feedback config if present
        if "feedback" in config_data:
            load_feedback_config_from_dict(config_data["feedback"])
