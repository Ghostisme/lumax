...
def _get_config_mtime() -> float | None:
    """Get the latest modification time for MCP-affecting config files.

    Returns:
        The latest modification time as a float, or None if no config exists.
    """
    mtimes: list[float] = []
    ...
    try:
        ...
        app_config_path = AppConfig.resolve_config_path()
        if app_config_path.exists():
            mtimes.append(os.path.getmtime(app_config_path))
    except Exception:
        logger.debug("Unable to read app config mtime for MCP cache invalidation", exc_info=True)
