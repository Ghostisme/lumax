...
from deerflow.metering import LumaxMeteringCallbackHandler
...
def create_chat_model(name: str | None = None, thinking_enabled: bool = False, *, app_config: AppConfig | None = None, **kwargs) -> BaseChatModel:
    ...
    callbacks = [LumaxMeteringCallbackHandler(), *build_tracing_callbacks()]
    if callbacks:
        existing_callbacks = model_instance.callbacks or []
        model_instance.callbacks = [*existing_callbacks, *callbacks]
        logger.debug("Model '%s' callbacks attached: %d", name, len(callbacks))
