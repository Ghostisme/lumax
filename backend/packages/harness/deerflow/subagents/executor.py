...
class SubagentExecutor:
    ...
    def execute(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            if loop is not None and loop.is_running():
                ...
                # Propagate ContextVars (incl. MCP user/tenant/business context) into isolated thread
                ctx = contextvars.copy_context()
                future = _isolated_loop_pool.submit(ctx.run, self._execute_in_isolated_loop, task, result_holder)
    ...
    def execute_async(self, task: str, task_id: str | None = None) -> str:
        ...
        # Capture ContextVars at submission time so MCP user/tenant/business context
        # propagates into the scheduler/execution thread pools.
        ...
        def run_task():
            ...
            try:
                ...
                # propagate ContextVars (e.g. MCP request context) via copy_context.
                exec_ctx = contextvars.copy_context()
                execution_future: Future = _execution_pool.submit(exec_ctx.run, self.execute, task, result_holder)
                try:
        ...
        _scheduler_pool.submit(parent_ctx.run, run_task)
        return task_id
