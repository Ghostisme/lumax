...
def test_localhost_memory_get_no_cookie_passes():
    """Local browser inspection of memory is allowed in dev without auth."""
    from fastapi import FastAPI
    ...
    res = local_client.get("/api/memory")

    assert res.status_code == 200
    assert res.json() == {"ok": True}


def test_non_localhost_memory_get_no_cookie_returns_401(client):
    res = client.get("/api/memory")

    assert res.status_code == 401


def test_localhost_memory_delete_no_cookie_returns_401():
    from fastapi import FastAPI
    ...
    res = local_client.delete("/api/memory")
