...
class TestFormatClarificationMessage:
    ...
    def test_oceanengine_clarification_hides_api_field_aliases(self, middleware):
        args = {
            "question": "请问您的本地推账号ID（localAccountId）是多少？",
            "clarification_type": "missing_info",
            "context": "需要使用本地推项目能力，不需要补充展示字段（payload_json 不需要用户提供）。多门店ID列表（multi_poi_ids）必须填写。",
        }

        result = middleware._format_clarification_message(args)

        assert "本地推账号ID" in result
        assert "localAccountId" not in result
        assert "payload_json" not in result
        assert "multi_poi_ids" not in result
