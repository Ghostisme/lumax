...
def test_features_mode(mock_create_agent):
    ...
    assert "OceanEngineResponseSanitizerMiddleware" in mw_types
    assert "ClarificationMiddleware" in mw_types
    sanitizer_index = next(i for i, mw in enumerate(middleware) if isinstance(mw, OceanEngineResponseSanitizerMiddleware))
