...
def _user_context():
    token = set_current_user(SimpleNamespace(id="test-user"))
    try:
        yield
    finally:
...
class TestFeedbackRepository:
    ...
    async def test_create_with_message_id(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        record = await repo.create(
            run_id="r1", thread_id="t1", rating=1, message_id="msg-42"
        )
    ...
    async def test_create_with_owner(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        record = await repo.create(
            run_id="r1", thread_id="t1", rating=1, user_id="user-1"
        )
        assert record["user_id"] == "user-1"
        await _cleanup()

    @pytest.mark.anyio
    async def test_create_neutral_rating_zero(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        record = await repo.create(
            run_id="r1",
            thread_id="t1",
        ...
        assert record["feedback_time"] is None
        await _cleanup()
    ...
    async def test_aggregate_by_run(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        await repo.create(run_id="r1", thread_id="t1", rating=0, user_id="user-0")
        await repo.create(run_id="r1", thread_id="t1", rating=1, user_id="user-1")
        await repo.create(run_id="r1", thread_id="t1", rating=1, user_id="user-2")
    ...
    @pytest.mark.anyio
    async def test_upsert_accepts_neutral_rating(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        record = await repo.upsert(
            run_id="r1", thread_id="t1", rating=0, user_id="u1", comment="ignored"
        )
        assert record["rating"] == 0
        assert record["result"] is None
    ...
    @pytest.mark.anyio
    async def test_ensure_neutral_for_run_creates_record(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        record = await repo.ensure_neutral_for_run(
            run_id="r-neutral",
            thread_id="t-neutral",
            user_id="u-neutral",
        ...
        assert record["result"] is None
        assert record["feedback_time"] is None
        assert record["agent_id"] == "assistant-1"
        assert record["agent_name"] == "custom-agent"
        await _cleanup()

    @pytest.mark.anyio
    async def test_ensure_neutral_for_run_persists_message_id(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        record = await repo.ensure_neutral_for_run(
            run_id="r-neutral-msg",
            thread_id="t-neutral-msg",
            user_id="u-neutral-msg",
            message_id="assistant-msg-1",
            agent_id="assistant-1",
            agent_name="custom-agent",
        )
        assert record["message_id"] == "assistant-msg-1"
        await _cleanup()

    @pytest.mark.anyio
    async def test_ensure_neutral_for_run_does_not_overwrite_message_id(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        first = await repo.ensure_neutral_for_run(
            run_id="r-neutral-msg",
            thread_id="t-neutral-msg",
            user_id="u-neutral-msg",
            message_id="assistant-msg-1",
        ...
        second = await repo.ensure_neutral_for_run(
            run_id="r-neutral-msg",
            thread_id="t-neutral-msg",
            user_id="u-neutral-msg",
        ...
        assert second["feedback_id"] == first["feedback_id"]
        assert second["message_id"] == "assistant-msg-1"
        await _cleanup()

    @pytest.mark.anyio
    async def test_ensure_neutral_for_run_does_not_overwrite_feedback(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        first = await repo.upsert_by_run(
            run_id="r1",
            thread_id="t1",
            user_id="u1",
            result="positive",
        ...
        assert second["feedback_id"] == first["feedback_id"]
        assert second["rating"] == 1
        assert second["result"] == "positive"
        assert second["comment"] == "good"
        assert second["agent_id"] == "assistant-1"
        await _cleanup()

    @pytest.mark.anyio
    async def test_upsert_by_run_updates_existing_neutral_record(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        neutral = await repo.ensure_neutral_for_run(
            run_id="r1",
            thread_id="t1",
            user_id="u1",
            agent_id="assistant-1",
        ...
        updated = await repo.upsert_by_run(
            run_id="r1",
            thread_id="t1",
            user_id="u1",
            result="negative",
        ...
        assert updated["feedback_id"] == neutral["feedback_id"]
        assert updated["rating"] == -1
        assert updated["result"] == "negative"
        assert updated["comment"] == "bad"
        assert updated["feedback_time"] is not None
        await _cleanup()

    @pytest.mark.anyio
    async def test_upsert_by_run_preserves_neutral_message_id(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        neutral = await repo.ensure_neutral_for_run(
            run_id="r1",
            thread_id="t1",
        ...
        assert updated["feedback_id"] == neutral["feedback_id"]
        assert updated["message_id"] == "assistant-msg-1"
        assert updated["result"] == "positive"
        await _cleanup()

    @pytest.mark.anyio
    async def test_reset_by_run_keeps_record_and_clears_feedback(self, tmp_path):
        repo = await _make_feedback_repo(tmp_path)
        created = await repo.upsert_by_run(
            run_id="r1",
            thread_id="t1",
            user_id="u1",
            result="positive",
        ...
        reset = await repo.reset_by_run(thread_id="t1", run_id="r1", user_id="u1")
        assert reset is not None
        assert reset["feedback_id"] == created["feedback_id"]
        assert reset["rating"] == 0
        assert reset["result"] is None
        assert reset["feedback_time"] is None
        await _cleanup()
...
class TestFollowUpAssociation:
    ...
    async def test_follow_up_auto_detection_logic(self):
        ...
        await store.put(
            "r2", thread_id="t1", status="error", created_at="2026-05-08T01:00:01+00:00"
        )
        ...
        await store.put(
            ...
            thread_id="t1",
            status="success",
            created_at="2026-05-08T01:00:02+00:00",
        )
        recent = await store.list_by_thread("t1", limit=1)
        follow_up = None
        if recent and recent[0].get("status") == "success":
