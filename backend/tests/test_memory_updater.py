...
@pytest.mark.anyio
async def test_aupdate_memory_injects_langfuse_session_id(monkeypatch):
    updater = MemoryUpdater()
    model = MagicMock()
    model.ainvoke = AsyncMock(return_value=MagicMock(content='{"newFacts": [], "factsToRemove": []}'))

    monkeypatch.setattr("deerflow.agents.memory.updater.create_chat_model", lambda **kwargs: model)
    monkeypatch.setattr(
        "deerflow.agents.memory.updater.get_memory_config",
        lambda: _memory_config(enabled=True, model_name=None),
    )
    monkeypatch.setattr(
        updater,
        "_prepare_update_prompt",
        lambda messages, agent_name, correction_detected, reinforcement_detected: (_make_memory(), "prompt"),
    )
    monkeypatch.setattr(
        updater,
        "_finalize_update",
        lambda current_memory, response_content, thread_id, agent_name: True,
    )
    ...
    assert model.ainvoke.await_args.kwargs["config"] == {
        "run_name": "memory_agent",
        "metadata": {"langfuse_session_id": "thread-memory"},
    }
