...
def test_delete_thread_route_cleans_thread_directory(tmp_path):
    ...
    paths.sandbox_work_dir("thread-route", user_id=user_id).mkdir(
        parents=True, exist_ok=True
    )
    (paths.sandbox_work_dir("thread-route", user_id=user_id) / "notes.txt").write_text(
    ...
    assert response.json() == {
        "success": True,
        "message": "Deleted local thread data for thread-route",
    }
    assert not thread_dir.exists()
...
def test_delete_thread_data_returns_generic_500_error(tmp_path):
    ...
    log_exception.assert_called_once_with(
        "Failed to delete thread data for %s", "thread-cleanup"
    )
...
async def test_search_threads_logs_user_context_and_result_count(caplog):
    ...
    context_user = SimpleNamespace(id="context-user")
    request = SimpleNamespace(
        ...
        state=SimpleNamespace(user=request_user),
        url=SimpleNamespace(path="/api/threads/search"),
    )

    token = set_current_user(context_user)
    try:
        with caplog.at_level(logging.INFO, logger="app.gateway.routers.threads"):
            response = await threads.search_threads(
                threads.ThreadSearchRequest(limit=50),
                request,
            )
    finally:
        reset_current_user(token)

    assert [item.thread_id for item in response] == ["thread-1"]
    log_output = "\n".join(record.getMessage() for record in caplog.records)
    assert "thread_search_request" in log_output
    assert "request_user_id=request-user" in log_output
    assert "context_user_id=context-user" in log_output
    assert "tenant_id=tenant-1" in log_output
    assert "business_code=talent" in log_output
    assert "thread_search_result" in log_output
    assert "row_count=1" in log_output
