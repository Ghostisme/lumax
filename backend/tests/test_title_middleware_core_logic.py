...
class TestTitleMiddlewareCoreLogic:
    ...
    def test_generate_title_uses_async_model_and_respects_max_chars(self, monkeypatch):
        ...
        monkeypatch.setattr(title_middleware_module, "get_config", lambda: {"configurable": {"thread_id": "thread-title"}})
        ...
        assert model.ainvoke.await_args.kwargs["config"] == {
            ...
            "configurable": {"thread_id": "thread-title"},
            "metadata": {"langfuse_session_id": "thread-title"},
        }
    ...
    def test_sync_fallback_title_drops_parameter_details(self):
        """Fallback title should summarize intent instead of storing raw payload details."""
        ...
        state = {
            "messages": [
                HumanMessage(
                    content=(
                        "帮我创建一个巨量引擎本地推的项目，信息如下："
                        "本地推投放账户ID(local_account_id): 123 - 营销目标(marketing_goal): VIDEO_IMAGE"
                    )
                ),
                AIMessage(content="好的"),
            ]
        ...
        assert result == {"title": "创建巨量引擎本地推项目"}
        assert "local_account_id" not in result["title"]
        assert "信息如下" not in result["title"]
    ...
    def test_generate_title_async_strips_think_tags_in_response(self, monkeypatch):
        ...
        monkeypatch.setattr(title_middleware_module, "create_chat_model", MagicMock(return_value=model))
        monkeypatch.setattr(title_middleware_module, "get_config", lambda: {})
    ...
    def test_get_runnable_config_injects_langfuse_session_id_from_parent_config(self, monkeypatch):
        middleware = TitleMiddleware()
        monkeypatch.setattr(
            title_middleware_module,
            "get_config",
            lambda: {"configurable": {"thread_id": "thread-title"}, "metadata": {"agent_name": "default"}},
        )

        config = middleware._get_runnable_config()

        assert config["metadata"]["agent_name"] == "default"
        assert config["metadata"]["langfuse_session_id"] == "thread-title"

    def test_get_runnable_config_preserves_explicit_langfuse_session_id(self, monkeypatch):
        middleware = TitleMiddleware()
        monkeypatch.setattr(
            title_middleware_module,
            "get_config",
        ...
        assert config["metadata"]["langfuse_session_id"] == "explicit-session"
