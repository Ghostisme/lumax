...
For any frontend implementation task, read `AGENT_CONVENTIONS.md` first, then apply this document and `CLAUDE.md` together.

## Governance Baseline

- 前端设计、代码和 skill 相关变更必须先走 OpenSpec，再进入 Apply。
- `frontend/src/app/**`、`frontend/src/core/**` 与 `frontend/src/components/**` 默认视为 DeerFlow 受保护源码；若进入更深目录，优先遵循更近的 `AGENTS.md`。
- `src/components/ui/` 与 `src/components/ai-elements/` 为生成目录，默认不手工修改。
- `src/app/mock/` 和纯内容目录不在本次包级 `AGENTS.md` 治理范围内。
