...
type SpeechRecognitionMode = "realtime" | "single-shot";

type SpeechRecognitionModeTarget = Pick<
  SpeechRecognition,
  "continuous" | "interimResults"
>;

const REALTIME_STREAMING_UNSUPPORTED_PATTERN =
  /does not support realtime streaming|not support realtime streaming/i;

export const shouldFallbackToSingleShotRecognition = (error: unknown) => {
  const message =
    typeof error === "string"
      ? error
  ...
  return REALTIME_STREAMING_UNSUPPORTED_PATTERN.test(String(message));
};

export const configureSpeechRecognitionMode = (
  speechRecognition: SpeechRecognitionModeTarget,
  mode: SpeechRecognitionMode,
) => {
  const realtime = mode === "realtime";
  speechRecognition.continuous = realtime;
  speechRecognition.interimResults = realtime;
};
...
export const PromptInputSpeechButton = ({
  ...
  const recognitionModeRef = useRef<SpeechRecognitionMode>("realtime");
  ...
  useEffect(() => {
    if (
      ...
      configureSpeechRecognitionMode(speechRecognition, "realtime");
      ...
      speechRecognition.onerror = (event) => {
        ...
        setIsListening(false);
        if (
          recognitionModeRef.current === "realtime" &&
          shouldFallbackToSingleShotRecognition(event)
        ) {
          recognitionModeRef.current = "single-shot";
          configureSpeechRecognitionMode(speechRecognition, "single-shot");
          toast.warning("当前语音识别不支持实时模式，已切换为单次识别。");
        } else {
          toast.error(event.message || event.error || "语音识别失败");
        }
      };
  ...
  const toggleListening = useCallback(() => {
    ...
    if (isListening) {
      ...
      try {
        ...
        if (
          recognitionModeRef.current === "realtime" &&
          shouldFallbackToSingleShotRecognition(error)
        ) {
          recognitionModeRef.current = "single-shot";
