...
| `NEXT_PUBLIC_BACKEND_BASE_URL`      | 否   | 覆盖前端使用的后端 API 地址     |
| `NEXT_PUBLIC_AUTH_API_BASE_URL`     | 否   | 覆盖认证 API 地址               |
| `NEXT_PUBLIC_LANGGRAPH_BASE_URL`    | 否   | 覆盖 LangGraph API 地址         |
| `NEXT_PUBLIC_STATIC_WEBSITE_ONLY`   | 否   | 启用静态网站模式                |

<Callout type="warning">
  不要在生产中使用 <code>SKIP_ENV_VALIDATION=1</code>。
</Callout>
