...
export function groupMessages<T>(
  ...
  let suppressNextAssistantContent = false;
  ...
  for (const message of messages) {
    if (message.type === "tool" && isClarificationToolMessage(message)) {
      // Clarification tool messages must remain visible as cards even when
      // backend marks the raw tool payload `hide_from_ui`.
      ...
      suppressNextAssistantContent = true;
      continue;
    }
    ...
    if (message.type === "human") {
      suppressNextAssistantContent = false;
    ...
    if (message.type === "tool") {
      ...
      if (open) {
        ...
        console.warn("Unexpected tool message outside a processing group", {
          id: message.id,
          name: message.name,
    ...
    if (message.type === "ai") {
      const shouldSuppressAssistantContent = suppressNextAssistantContent;
...
export function isClarificationToolMessage(message: Message) {
  return (
    ...
    (message.name === "ask_clarification" ||
      message.name === "structured_clarification")
  );
}
