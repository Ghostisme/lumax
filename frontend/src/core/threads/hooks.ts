...
export function normalizeStoredRunId(runId: string | null): string | null {
  if (!runId) {
    return null;
  }
...
export function getRunMetadataStorage(): {
...
function getReconnectRunId(
  storage: ReturnType<typeof getRunMetadataStorage> | undefined,
  threadId: string,
): string | null {
  if (!threadId) {
    return null;
  }
  return storage?.getItem(`lg:stream:${threadId}`) ?? null;
}
...
export function useThreadStream({
  ...
  useEffect(() => {
    ...
    if (
      normalizedThreadId &&
      normalizedThreadId === optimisticThreadIdRef.current
    ) {
      return;
    }
  ...
  const sendMessage = useCallback(
    async (
      ...
      const reconnectRunId = getReconnectRunId(
        runMetadataStorageRef.current,
        threadId,
      );
      if (reconnectRunId) {
        try {
          await thread.joinStream(reconnectRunId);
        } finally {
          sendInFlightRef.current = false;
          optimisticThreadIdRef.current = null;
          setOptimisticMessages([]);
